...
if($_POST['type'] == 'list'){
    $stmt = $conn->prepare("SELECT `id`,`user`,`image_url`,`instagram_count`,`instagram_url`,`twitter_count`,`twitter_url`,`facebook_count`,`facebook_url`,`total` FROM `Influencer_Information` WHERE `id` IN ('$comma_separated') AND `user` LIKE ?  ORDER BY `total` DESC LIMIT $page,10");
    ...
    while($stmt->fetch()){
        ...
        $page = $_POST['page'] * 12;
        $comma_separated = implode("','", $influencerlist);
        $stmt = $conn->prepare("SELECT `id`,`user`,`image_url`,`instagram_count`,`instagram_url`,`twitter_count`,`twitter_url`,`facebook_count`,`facebook_url`,`total` FROM `Influencer_Information` WHERE `id` IN ('$comma_separated') AND `user` LIKE ?  ORDER BY `total` DESC LIMIT $page,12");
        $stmt->bind_param('s',$query);
        $stmt->execute();
        $stmt->bind_result($id,$user,$image,$instagramcount,$instagramurl,$twittercount,$twitterurl,$facebookcount,$facebookurl,$total);
        $count = 4;
